...
<div class="grid grid-cols-2 min-[481px]:grid-cols-[repeat(auto-fit,minmax(140px,1fr))] min-[721px]:grid-cols-[repeat(auto-fit,minmax(180px,1fr))] gap-4 mb-8">
    <div class="bg-white border border-border rounded-2xl p-3 text-center shadow-sm no-underline text-inherit max-w-[220px] transition-colors hover:border-primary">
        ...
        <div class="font-cursive text-[0.9rem] text-success">Commandes totales</div>
    ...
    <div class="bg-white border border-border rounded-2xl p-3 text-center shadow-sm no-underline text-inherit max-w-[220px] transition-colors hover:border-primary">
        ...
        <div class="font-cursive text-[1.7rem] font-bold text-success leading-none mb-1"><?= number_format($stats['in_progress'], 0, ',', ' ') ?></div>
        <div class="font-cursive text-[0.9rem] text-success">En cours</div>
    </div>

    <div class="bg-white border border-border rounded-2xl p-3 text-center shadow-sm no-underline text-inherit max-w-[220px] transition-colors hover:border-primary">
        <img class="w-20 h-20 object-contain mx-auto" src="/assets/images/icones/commandes-terminees.png" alt="">
        <div class="font-cursive text-[1.7rem] font-bold text-success leading-none mb-1"><?= number_format($stats['completed'], 0, ',', ' ') ?></div>
        <div class="font-cursive text-[0.9rem] text-success">Terminées</div>
    </div>

    <div class="bg-white border border-border rounded-2xl p-3 text-center shadow-sm no-underline text-inherit max-w-[220px] transition-colors hover:border-primary">
        <img class="w-20 h-20 object-contain mx-auto" src="/assets/images/icones/commandes-annulees.png" alt="">
        <div class="font-cursive text-[1.7rem] font-bold text-success leading-none mb-1"><?= number_format($stats['cancelled'], 0, ',', ' ') ?></div>
        <div class="font-cursive text-[0.9rem] text-success">Annulées</div>
    </div>

    <div class="bg-white border border-border rounded-2xl p-3 text-center shadow-sm no-underline text-inherit max-w-[220px] transition-colors hover:border-primary">
        <img class="w-20 h-20 object-contain mx-auto" src="/assets/images/icones/commissions.png" alt="">
        <div class="font-cursive text-[1.7rem] font-bold text-success leading-none mb-1"><?= number_format($stats['total_revenue'] / 100, 2, ',', ' ') ?>€</div>
    ...
    <a href="/admin/orders?archived=only" class="bg-white border border-border rounded-2xl p-3 text-center shadow-sm no-underline text-inherit max-w-[220px] transition-colors hover:border-primary" title="Voir les commandes archivées">
        <svg class="w-20 h-20 object-contain mx-auto" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" style="color: var(--color-primary-muted);">
            <rect x="3" y="4" width="18" height="4" rx="1"></rect>
            <path d="M5 8v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V8"></path>
            <line x1="10" y1="12" x2="14" y2="12"></line>
        </svg>
        <div class="font-cursive text-[1.7rem] font-bold text-success leading-none mb-1"><?= number_format($stats['archived'], 0, ',', ' ') ?></div>
        <div class="font-cursive text-[0.9rem] text-success">Archivées</div>
    </a>
</div>
...
<div class="bg-white border border-border rounded-md overflow-hidden shadow-sm">
    ...
    <?php else: ?>
        <div class="overflow-x-auto">
            <table class="data-table max-[720px]:min-w-[640px] [&_input]:w-4 [&_input]:h-4 [&_input]:accent-primary [&_input]:cursor-pointer" data-bulk-table data-bulk-form="ordersBulkForm">
                <thead>
                    <tr>
                        <th>
